...
> [TypeScript](/solutions/typescript/2019/12/part_one.ts)\
...
Callisto has a `x` position of `5`, then Ganymede's `x` velocity **changes**
**by +1** (because `5 > 3`) and Callisto's `x` velocity **changes by -1**
(because `3 < 5`). However, if the positions on a given axis are the same, the
...
energy for a single moon is its **potential energy** multiplied by its
**kinetic energy**. A moon's **potential energy** is the sum of the
...
Your puzzle answer was `8625`.

## [Part Two](https://adventofcode.com/2019/day/12#part2)

> [TypeScript](/solutions/typescript/2019/12/part_two.ts)\
> [Rust](/solutions/rust/2019/12/src/lib.rs)
...
Your puzzle answer was `332477126821644`.
