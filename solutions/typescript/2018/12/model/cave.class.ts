export class Cave {
	...
	public normalize(): void {
		...
		this.offset += start - 4;
		this.row =
			'.'.repeat(Math.max(0, 4 - start)) +
			this.row.slice(Math.max(0, start - 4), this.row.length);
		this.row =
			this.row.slice(0, this.row.length - Math.max(0, end - 4)) +
			'.'.repeat(Math.max(0, 4 - end));
	}
	...
	public countFromEnds(input: string, match: string, invert = false): number {
		let i = 0;
