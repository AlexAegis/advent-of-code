...
describe('2019 - Day 7 - Part One', () => {
	it(`should resolve to 929800 when using the input`, async () => {
		...
		expect(p1(resources.input)).to.equal(929800);
	});

	it('should be that that both the first examples resolves to 43210', async () => {
		expect(await p1('3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0')).to.equal(43210);
	});

	it('should be that that both the first examples resolves to 54321', async () => {
		expect(
			await p1('3,23,3,24,1002,24,10,24,1002,23,-1,23,101,5,23,23,1,24,23,23,4,23,99,0,0')
		).to.equal(54321);
	});
