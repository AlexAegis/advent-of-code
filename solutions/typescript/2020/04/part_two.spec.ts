...
describe('2020 - Day 4 - Part Two', () => {
	...
	it('should solve the all valid example', async () => {
		expect(runner((await read(year, day, 'example.valid.txt')()).input)).to.equal(4);
	});

	it('should solve the all invalid example', async () => {
		expect(runner((await read(year, day, 'example.invalid.txt')()).input)).to.equal(0);
	});

	it('should solve for the second input', async () => {
		expect(runner((await read(year, day, 'input.2.txt')()).input)).to.equal(198);
	});
