...
describe('snailfish numbers', () => {
	describe('magnitude', () => {
		...
		it('should be 3488 for [[[[8,7],[7,7]],[[8,6],[7,7]]],[[[0,7],[6,6]],[8,7]]]', () => {
			...
			expect(getMagnitude(number)).toEqual(3488);
		});
		...
		it('should be 4140 for the last example', () => {
			...
			expect(sum.toString()).toEqual(
				'[[[[6,6],[7,6]],[[7,7],[7,0]]],[[[7,7],[7,7]],[[7,8],[9,9]]]]',
			);
			expect(getMagnitude(sum)).toEqual(4140);
		});
	});
	...
	describe('addition', () => {
		describe('with the detailed example', () => {
			...
			it('should work in a set order', () => {
				...
				expect(result.toString()).toEqual('[[[[0,7],4],[[7,8],[6,0]]],[8,1]]');
			});
		});
		...
		describe('with simple numbers', () => {
			...
			it('should be able to add up the first 4', () => {
				...
				expect(result.toString()).toEqual('[[[[1,1],[2,2]],[3,3]],[4,4]]');
			});
			...
			it('should be able to add up the first 5', () => {
				...
				expect(result.toString()).toEqual('[[[[3,0],[5,3]],[4,4]],[5,5]]');
			});
			...
			it('should be able to add up all 6 of them', () => {
				...
				expect(result.toString()).toEqual('[[[[5,0],[7,4]],[5,5]],[6,6]]');
			});
		});
	...
	describe('explosion', () => {
		it('should explode [[[[[9,8],1],2],3],4] into [[[[0,9],2],3],4]', () => {
			...
			expect(exploded).toEqual(true);
			expect(number.toString()).toEqual('[[[[0,9],2],3],4]');
			exploded = triggerExplosion(number);
			expect(exploded).toEqual(false);
		});
		...
		it('should explode [7,[6,[5,[4,[3,2]]]]] into [7,[6,[5,[7,0]]]]', () => {
			...
			let exploded = triggerExplosion(number);
			expect(exploded).toEqual(true);
			expect(number.toString()).toEqual('[7,[6,[5,[7,0]]]]');
			exploded = triggerExplosion(number);
			expect(exploded).toEqual(false);
		});
		...
		it('should explode [[6,[5,[4,[3,2]]]],1] into [[6,[5,[7,0]]],3]', () => {
			...
			let exploded = triggerExplosion(number);
			expect(exploded).toEqual(true);
			expect(number.toString()).toEqual('[[6,[5,[7,0]]],3]');
			exploded = triggerExplosion(number);
			expect(exploded).toEqual(false);
		});
		...
		it('should explode [[3,[2,[1,[7,3]]]],[6,[5,[4,[3,2]]]]] into [[3,[2,[8,0]]],[9,[5,[4,[3,2]]]]]', () => {
			...
			expect(exploded).toEqual(true);
			expect(number.toString()).toEqual('[[3,[2,[8,0]]],[9,[5,[4,[3,2]]]]]');
		});
		...
		it('should explode [[3,[2,[8,0]]],[9,[5,[4,[3,2]]]]] into [[3,[2,[8,0]]],[9,[5,[7,0]]]]', () => {
			...
			const exploded = triggerExplosion(number);
			expect(exploded).toEqual(true);
			expect(number.toString()).toEqual('[[3,[2,[8,0]]],[9,[5,[7,0]]]]');
		});
	});
});
