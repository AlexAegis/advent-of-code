...
describe('2022 04 p1', () => {
	describe('the input', () => {
		it('should solve the input', async () => {
			...
			expect(p1(resources.input)).to.equal(433);
		});
	});
	...
	describe('isFullyOverlapping', () => {
		it('should be true when the second is fully inside the first', () => {
			expect(areClosedRangesFullyOverlapping([2, 8], [3, 7])).to.be.true;
		});

		it('should be true when the first is fully inside the second', () => {
			expect(areClosedRangesFullyOverlapping([4, 6], [3, 7])).to.be.true;
		});

		it('should be true when the first is the same as the second', () => {
			expect(areClosedRangesFullyOverlapping([4, 6], [4, 6])).to.be.true;
		});

		it('should be true when the only one side is the same as in the other', () => {
			expect(areClosedRangesFullyOverlapping([4, 7], [4, 6])).to.be.true;
		});

		it('should be true when every value is the same', () => {
			expect(areClosedRangesFullyOverlapping([1, 1], [1, 1])).to.be.true;
		});

		it('should be true when the first range has one element', () => {
			expect(areClosedRangesFullyOverlapping([1, 1], [1, 5])).to.be.true;
		});

		it('should be false when they are not overlapping at all', () => {
			expect(areClosedRangesFullyOverlapping([1, 2], [3, 4])).to.be.false;
		});

		it('should be false when they are only partially overlapping', () => {
			expect(areClosedRangesFullyOverlapping([1, 5], [3, 8])).to.be.false;
		});
	});
});
