 ...
 *
 * @param n the number where digit
 * @param p will be extracted from
 *
 * Implementation:
 *
 * `Number.parseInt(n.toString()[p] ?? '0', 10)`
 *
 * This string based solution is faster than using a purely mathematical solution
 *
 * Alternative implementation for reference:
 *
 * `Math.floor((n / Math.pow(10, Math.floor(Math.log10(Math.abs(n))) - p)) % 10)`
 *
