...
describe(`${year} - Day ${day} - Part One`, () => {
	it(`should resolve to ${results.one.example0} when using the zeroth example`, async () => {
		expect(await runner((await reader(year, day, 'example.0.txt')()).input)).to.equal(results.one.example0);
	});
	...
	it(`should resolve to ${results.one.example3} when using the third example`, async () => {
		expect(await runner((await reader(year, day, 'example.3.txt')()).input)).to.equal(results.one.example3);
	});

	it(`should resolve to ${results.one.example4} when using the fourth example`, async () => {
		expect(await runner((await reader(year, day, 'example.4.txt')()).input)).to.equal(results.one.example4);
	});

	it(`should resolve to ${results.one.example5} when using the fifth example`, async () => {
		expect(await runner((await reader(year, day, 'example.5.txt')()).input)).to.equal(results.one.example5);
	});
